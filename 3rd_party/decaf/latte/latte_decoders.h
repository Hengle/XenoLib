...
struct AluGroup
{
   AluGroup(const latte::AluInst *group)
   {
      ...
      instructions = std::span(group, instructionCount);
      literals = std::span(reinterpret_cast<const uint32_t *>(group + instructionCount), literalCount);
   }
   ...
   std::span<const latte::AluInst> instructions;
   std::span<const uint32_t> literals;
};
...
{
public:
   AluClauseParser(std::span<const AluInst> slots, bool aluInstPreferVector)
      ...
      readOneGroup()
   {
      ...
      if (literalCount > 0) {
         ...
         group.literals = std::span(reinterpret_cast<const uint32_t*>(&literalSpan[0]), literalCount);
